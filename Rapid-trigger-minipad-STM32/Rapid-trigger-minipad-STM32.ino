/*
  ...
  当前版本：v1.3，测试用开发板为STM32 F103C8T6，上传程序前请确认以下设置是正确的：
...
#define         KEYS                           4                                          // 按键数
#define         LIGHT                       PC13                                          // 指示灯，插入后板子上的蓝色指示灯会亮起一段时间，这时不能按下按键，这是在初校准。如果需更改，注意是置低点亮，置高熄灭
...
void initKey() {
  ...
  for (int n = 0; n < KEYS; n++) {
    keys[n].cushion = (keys[n].max - keys[n].min) * times[n];   // 计算缓冲值
